...
for size in ${NOTOSANS_FONT_SIZES[@]}; do
  for style in ${READER_FONT_STYLES[@]}; do
    ...
    python fontconvert.py $font_name $size $font_path $hebrew_fallback --2bit --compress --pnum --additional-intervals 0x0590,0x05FF --additional-intervals 0xFB1D,0xFB4F > $output_path
...
for size in ${UI_FONT_SIZES[@]}; do
  for style in ${UI_FONT_STYLES[@]}; do
    ...
    hebrew_fallback="${!hebrew_fallback_var}"
    output_path="../builtinFonts/${font_name}.h"
    python fontconvert.py $font_name $size $font_path $hebrew_fallback --additional-intervals 0x0590,0x05FF --additional-intervals 0xFB1D,0xFB4F > $output_path
    echo "Regenerated $output_path (with Hebrew fallback)"
  done
done

python fontconvert.py notosans_8_regular 8 ../builtinFonts/source/NotoSans/NotoSans-Regular.ttf ../builtinFonts/source/NotoSansHebrew/NotoSansHebrew-Regular.ttf --additional-intervals 0x0590,0x05FF --additional-intervals 0xFB1D,0xFB4F > ../builtinFonts/notosans_8_regular.h
