...
# Pass Hebrew + Alphabetic Presentation Forms ranges via --additional-intervals
# (avoids modifying upstream's `intervals` list in fontconvert.py).
$hebrewIntervals = '--additional-intervals 0x0590,0x05FF --additional-intervals 0xFB1D,0xFB4F'
...
foreach ($size in $notoSizes) {
  foreach ($style in $readerStyles) {
    ...
    cmd /c "python fontconvert.py $fontName $size $fontPath $hebrewFallback --2bit --compress --pnum $hebrewIntervals > $outPath"
...
foreach ($size in $uiSizes) {
  foreach ($style in $uiStyles) {
    ...
    $outPath = "../builtinFonts/${fontName}.h"
    cmd /c "python fontconvert.py $fontName $size $fontPath $hebrewFallback $hebrewIntervals > $outPath"
    if ($LASTEXITCODE -ne 0) { throw "fontconvert failed for $fontName" }
    Write-Host "Regenerated $outPath (with Hebrew fallback)"
  }
}

cmd /c "python fontconvert.py notosans_8_regular 8 ../builtinFonts/source/NotoSans/NotoSans-Regular.ttf $hebrewNotoRegular $hebrewIntervals > ../builtinFonts/notosans_8_regular.h"
